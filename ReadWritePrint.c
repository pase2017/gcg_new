...
PetscErrorCode ReadScaleMatrixBinary(Mat *A)
{
    ...
    ierr = PetscOptionsGetString(NULL, NULL, "-mat_scale", filename, sizeof(filename), NULL); CHKERRQ(ierr);
    PetscPrintf(PETSC_COMM_WORLD, "-mat_scale filename: %s\n", filename);
    ierr = ReadPetscMatrixBinary(A, filename); CHKERRQ(ierr);
...
PetscErrorCode ReadGenaralMatrixBinary(Mat *A, Mat *B, PetscInt ifBI)
{
    ...
    if(ifBI == 1)
    {
        ...
        ierr = PetscOptionsGetInt(NULL,NULL,"-n_BI",&n,NULL);CHKERRQ(ierr);
...
{
    PetscErrorCode ierr;
    PetscInt       i, j, nrows, Istart, Iend;
    PetscScalar    *y;
